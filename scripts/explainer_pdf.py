...
TOP_K = 5
SIMILARITY_THRESHOLD = 250  # Lower = more similar (distance)
...
def retrieve_context(query, k=TOP_K, category_filter="nephrology"):
    ...
    return sorted(filtered, key=lambda x: x["score"]) if filtered else sorted(context, key=lambda x: x["score"])
...
#File Parsing + Batch Explanation
def extract_text_from_file(file):
    filepath = file.name if hasattr(file, "name") else file  # supports both Gradio + CLI
...
def extract_lab_terms(text):
    pattern = r"(?i)\b(gfr|bun|creatinine|albumin|potassium|sodium|urea|uric acid|phosphorus|egfr)[^\d]{0,20}?(\d+\.?\d*)"
    matches = re.findall(pattern, text)
    return [f"{term.upper()} {value}" for term, value in matches]

def explain_lab_file(file):
    raw_text = extract_text_from_file(file)
    terms = extract_lab_terms(raw_text)

    explanations = []
    for term in terms:
        try:
            result = explain_medical_term(term)
            explanations.append(f"### {term}\n{result['explanation']}\n")
        except Exception as e:
            explanations.append(f"### {term}\n Error: {e}\n")

    return "\n".join(explanations) if explanations else "No known lab terms found in the file."
